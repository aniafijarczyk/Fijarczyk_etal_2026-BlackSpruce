rm(list=ls())
setwd("C:/Users/aniaf/Projects/BlackSpruce/948_garden_offset_run_separate")
...
prepare_inputs <- function(trait, filepath, garden, suffix) {
  ...
  # Filtering samples from snp matrix
...
for (trait in traits) {
  ...
  for (dset in c("100","1000","lfmm")) {
    ...
    for (garden in c("AC","ML","CH","PR")) {
      ...
      run.gradient <- try(run.gradient <- run_gradient_forest(run.inputs[["meta"]], run.inputs[['snps']], trait, garden, dset), silent=TRUE)
      if (inherits(run.gradient, "try-error")) {
        message("Skipped element due to error")
        next
      }
      
      run.offset <- calculate_genetic_offset(run.gradient, trait, garden, dset)           
