rm(list=ls())
setwd("C:/Users/aniaf/Projects/BlackSpruce/960B_run_rda_clusters_var5")
...
calculate_genetic_offset <- function(datasets_meta, datasets_snps, garden, group_name, trait) {
...
run.offset <- calculate_genetic_offset(run.inputs[["meta"]], run.inputs[['snps']], garden, group_name, trait)
run.offset %>% head()
...
for (trait in traits) {
  ...
  for (garden in c("AC","CH","ML","PR")) {
    for (group_name in c("East", "Central", "West")) {
      ...
      run.gradient <- try(calculate_genetic_offset(run.inputs[["meta"]], run.inputs[['snps']], garden, group_name, trait), silent=TRUE)
