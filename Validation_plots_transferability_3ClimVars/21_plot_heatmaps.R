...
df1 <- read.csv("19_bootstrap.tsv", sep="\t", header=T)
dim(df1)
df1 <- df1 %>% filter(n_samples >=5) %>% drop_na()
head(df1)
dim(df1)

df2 <- read.csv("20_bootstrap_clusters.tsv", sep="\t", header=T)
dim(df2)
df2 <- df2 %>% filter(n_samples >=5) %>% drop_na()
dim(df2)
head(df2)

df3 <- read.csv("./02B_bootstrap_rda.tsv", sep="\t", header=T)
df3 <- df3 %>% filter(n_samples >=5) %>% drop_na() %>% filter(marker == "1000var5")
dim(df3)
head(df3)
...
df4 <- read.csv("./03B_bootstrap_clusters_rda.tsv", sep="\t", header=T)
df4 <- df4 %>% filter(n_samples >=5) %>% drop_na() %>% filter(marker == "1000var5")
head(df4)
...
png("21_plot_heatmaps_RDA.png", h = 3000, w = 3000, res=300)
...
png("21_plot_heatmaps_selected_RDA.png", w = 2200, h = 1000, res=300)
p2
dev.off()
