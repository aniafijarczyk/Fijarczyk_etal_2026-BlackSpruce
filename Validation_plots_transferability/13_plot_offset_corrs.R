...
df$train_group1 <- factor(df$train_group1, levels = unique(df$train_group1))
df$train_group2 <- factor(df$train_group2, levels = unique(df$train_group2))
head(df)
...
p1 <- ggplot(df_sub, aes(x = train_group1, y = train_group2, fill = r)) +
...
p2 <- ggplot(df.sub, aes(x = train_group1, y = train_group2, fill = r)) +
  geom_tile(color = "white") +
  ...
  labs(x = "Cluster trained on", y = "Cluster trained on")
p2



#png("13_plot_offset_corrs.png", w = 3000, h = 4000, res=300)
#p2
#dev.off()
...
p3 <- ggplot(df.sub, aes(x = train_group1, y = train_group2, fill = r)) +
  geom_tile(color = "white") +
  geom_text(aes(label=round(r,2)), color="black", size=3) +
  scale_fill_distiller(palette = "RdYlBu", limits = c(-1, 1),
                       name = "Pearson r", direction=-1) +
...
png("13_plot_offset_corrs.png", w = 3000, h = 3500, res=300)
