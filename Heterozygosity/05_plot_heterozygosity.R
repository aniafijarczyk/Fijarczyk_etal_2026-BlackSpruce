...
df.all <- read.csv("04_calculate_heterozygosity_all_sites.tsv", sep="\t", header=T)
head(df.all)
...
head(dm)
write.table(dm, "05_plot_heterozygosity.tsv", sep="\t", col.names = T, row.names = F, quote=F, append=F)
...
png("05_plot_heterozygosity.png", w = 1100, h = 1000, res=300)
p2
dev.off()
...
png("05_plot_heterozygosity_He.png", w = 1600, h = 1200, res=300)
p4
dev.off()
