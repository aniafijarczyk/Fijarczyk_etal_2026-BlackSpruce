rm(list=ls())
setwd("C:/Users/aniaf/Projects/BlackSpruce/992_run_rda_exclude_pops")
...
calculate_genetic_offset <- function(datasets_meta, datasets_snps, garden, trait, n_samples, iteration) {
...
run.offset <- calculate_genetic_offset(run.inputs[["meta"]], run.inputs[['snps']], garden, trait, n_samples, iteration)
    ...
    for (n_samples in seq(2,42, by=2)) {
      ...
      for (iteration in c(1,2,3,4,5)) {
        ...
        if (run.inputs$check == 'ok') {
          run.gradient <- try(calculate_genetic_offset(run.inputs[["meta"]], run.inputs[['snps']], garden, trait, n_samples, iteration), silent=TRUE)
