...
prepare_inputs <- function(trait, filepath, garden, suffix) {
  ...
  # Sorting
  df.sort <- data.frame("sample" = rownames(dff.snps))
  ...
  print(sum(rownames(dff.snps) == df.sort.current.train$sample)/length(df.sort.current.train$sample))
  
  output <- list()
  output[["snps"]] <- dff.snps
...
run.inputs$snps[1:10,1:10]
dim(run.inputs$snps)

# running rda model - modeling current allele frequencies as a function of current climate
rda_model <- rda(run.inputs$snps ~ PC1 + PC2 + PC3, data = run.inputs$meta)
rda_model
rda_model$CCA$biplot
