rm(list=ls())
setwd("C:/Users/aniaf/Projects/BlackSpruce/991_run_rda_separate")
dir()
...
filename.2 = paste0(path, filecore, "1000", "_", trait, ".tsv")
print(filename.2)


run.inputs <- prepare_inputs(trait, filename, garden, dset)
run.offset <- calculate_genetic_offset(run.inputs[["meta"]], run.inputs[['snps']], trait, garden, dset)
run.offset$offset_rda

run.inputs.2 <- prepare_inputs(trait, filename.2, garden, dset)
run.offset.2 <- calculate_genetic_offset(run.inputs.2[["meta"]], run.inputs.2[['snps']], trait, garden, dset)
run.offset.2$offset_rda

plot(run.offset$offset_rda, run.offset.2$offset_rda)
...
for (trait in traits) {
  ...
  for (dset in c("100","1000","10000","lfmm")) {
    print(dset)
    ...
    for (garden in c("AC","ML","CH","PR")) {
      ...
      run.offset <- try(run.offset <- calculate_genetic_offset(run.inputs[["meta"]], run.inputs[['snps']], trait, garden, dset), silent=TRUE)
...
for (trait in traits) {
  ...
  for (dset in c("all")) {
    ...
    for (garden in c("AC","ML","CH","PR")) {
      ...
      path = "../945_GEA_lfmm/"
      filecore = "02_lea_inputs"
      filename.2 = paste0(path, filecore, "_", trait, ".tsv")
      print(filename.2)
      
      run.inputs <- prepare_inputs(trait, filename.2, garden, dset)
...
for (trait in traits) {
  ...
  for (dset in c("100","1000")) {
    suffix <- paste0(dset, "LF")
    print(suffix)
    
    for (garden in c("AC","ML","CH","PR")) {
      print(garden)
      
      path = "../45B_low_frequency/"
      filecore = "01_frequencies_POP_"
      filename.2 = paste0(path, filecore, dset, "_", trait, ".tsv")
      print(filename.2)
      
      run.inputs <- prepare_inputs(trait, filename.2, garden, suffix)
      
      run.offset <- try(run.offset <- calculate_genetic_offset(run.inputs[["meta"]], run.inputs[['snps']], trait, garden, suffix), silent=TRUE)
      if (inherits(run.offset, "try-error")) {
        message("Skipped element due to error")
        next
      }  
