...
df1 <- read.csv("./05_garden_geno.tsv", sep="\t", header=T)
df1$Model <- "GF"
df1 <- df1 %>% filter(n_samps >=5)
head(df1)

df2 <- read.csv("./06_garden_geno_rda.tsv", sep="\t", header=T)
df2$Model <- "RDA"
df2 <- df2 %>% filter(n_samps >=5)
head(df2)
...
df.sub <- dm %>% filter(trait %in% traits) %>% filter(upper_limit %in% c(2,4,6,8,10,12,14))
df.sub$trait_labels <- factor(df.sub$trait, levels = traits, labels = trait.labels)
head(df.sub)
unique(df.sub$upper_limit)
df.sub$SITE_ID <- factor(df.sub$SITE_ID, levels = c("PR","ML","CH","AC"))
head(df.sub)

p1 <- ggplot(df.sub) + aes(x = upper_limit, y = spearman_rho, color=SITE_ID, shape = Model) +
  ...
  labs(x = "Train population sample size", y = "Spearman rho") + 
...
#png("07_plot_correlations_geno_all.png", h = 2900, w = 3200, res=300)
#p1
#dev.off()
...
head(df.sub)
df.means <- df.sub %>%
  group_by(upper_limit, SITE_ID, trait_labels, Model) %>%
...
p2 <- ggplot(df.sub) +
  ...
  geom_point(data=df.sub,
             aes(x = upper_limit, y = spearman_rho, color=SITE_ID, shape = Model),
             size=1,
             alpha=0.2) +
  
  geom_line(data = df.means, 
            aes(x = upper_limit, y = mean_spearman, color=SITE_ID, linetype=Model,
  ...
  labs(x = "Maximum population size in the training dataset", y = "Spearman rho") + 
...
png("07_plot_correlations_pops_all_II.png", h = 2900, w = 3200, res=300)
p2
dev.off()
