rm(list=ls())
setwd("C:/Users/aniaf/Projects/BlackSpruce/975_provenances_offset_run_clusters_var3")
...
prepare_inputs <- function(filepath, group_name) {
...
run_gradient_forest <- function(datasets_meta, datasets_snps, group_name) {
  ...
  save(gf, file = paste0("./data/02_run_gradient_forest_",group_name,".RData"))
  return(gf)
}
...
calculate_genetic_offset <- function(gradient_forest_output, group_name, year, scenario) {
  ...
  # Saving table
  write.table(dout, file = paste0("02_run_gradient_forest_",group_name,"_",year,"_",scenario,".tsv"),
...
# group_name = "West"
# 
# inputs <- prepare_inputs(paste0(path, "01_frequencies_POP.tsv"), group_name)
# inputs
# dim(inputs$snps)
# dim(inputs$meta)
# head(inputs$meta)
# 
# gradient <- run_gradient_forest(inputs[["meta"]], inputs[['snps']], group_name)
# gradient
# 
# offset <- calculate_genetic_offset(gradient, group_name, "2031", "245")
# head(offset)
...
path = "../926_MoransI/"
inputs <- prepare_inputs(paste0(path, "01_moran_Pop_AFs.csv"), "West")

for (group_name in c("West","Central","East")) {
  
  inputs <- prepare_inputs(paste0(path, "01_moran_Pop_AFs.csv"), group_name)
  
  gradient <- try(run_gradient_forest(inputs[["meta"]], inputs[['snps']], group_name), silent=TRUE)
  ...
  for (year in c(2031, 2050, 2070, 2090)) {
    for (scenario in c("245")) {
      ...
      print(paste(group_name, year, scenario))
      offset <- calculate_genetic_offset(gradient, group_name, year, scenario)
